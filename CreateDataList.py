...
class CreateDataList:
    ...
    def createDataList(self, data_root_path):
        ...
        for class_dir in class_dirs:
            ...
            test_sum = 0
            trainer_sum = 0
            # 将数据列表直接保存在 data_root_path (例如 data/face/)
            # 统计每个类别有多少张图片
            class_sum = 0
            # 获取类别路径
            ...
            for img_path in img_paths:
                ...
                isexist = os.path.exists(data_root_path) # 检查 data_root_path 是否存在
                if not isexist:
                    os.makedirs(data_root_path) # 如果不存在则创建
                # 每10张图片取一个做测试数据
                if class_sum % 10 == 0:
                    test_sum += 1
                    with open(os.path.join(data_root_path, "test.list"), 'a') as f:
                        f.write(name_path + "\t%d" % class_label + "\n")
                else:
                    trainer_sum += 1
                    with open(os.path.join(data_root_path, "trainer.list"), 'a') as f:
                        f.write(name_path + "\t%d" % class_label + "\n")
                class_sum += 1
                all_class_images += 1
            class_label += 1
        ...
        with open(os.path.join(data_root_path, "readme.json"),'w') as f:
            f.write(jsons)
...
if __name__ == '__main__':
    ...
    # 使用data/face目录作为数据源
    createDataList.createDataList('data/face') 